  ...
  try {
    ...
    if (userId !== 'user-dev-uuid' && userId !== 'admin-dev-uuid' && isDbConfigured) {
      ...
      if (checkpoint) {
        ...
        const triggerThreshold = parseFloat(checkpoint.trigger_balance as any);
        if (activeBalance < triggerThreshold) {
          return res.status(400).json({
            ...
            profitOverride: parseFloat(checkpoint.profit_override as any) || 0.00
          });
        }

        // Apply custom profit commission if checkpoint specifies override
        if (parseFloat(checkpoint.profit_override as any) > 0) {
          payoutEarned = parseFloat(checkpoint.profit_override as any);
        }
      }
    }
